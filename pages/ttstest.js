...
export default function TTSTestPage() {
  ...
  // --- 話者サンプルの試聴ハンドラー（Supabase Storage から再生） ---
  const handlePreviewVoice = (voice) => {
    ...
    const audioUrl = getVoiceSampleUrl(voice.name)
    playPreviewAudio(audioUrl, voice.name)
  }
  ...
  // --- 話者サンプルのローカル保存ハンドラー（Supabase Storage からダウンロード） ---
  const handleSavePreview = async (voice) => {
    setPreviewSavingVoice(voice.name)
    try {
      const url = getVoiceSampleUrl(voice.name)
      ...
      console.error('音声保存エラー:', err)
      window.open(getVoiceSampleUrl(voice.name), '_blank')
    } finally {
