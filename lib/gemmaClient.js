...
[KEY_TAG]
[キャラとの現在の会話状況・シーンで画像を生成するうえで最も重要な要素を表す英語タグを1つだけ自ら考えて出力]
[IMAGE_PROMPT]
[指定された各カテゴリーから1つずつ選択した候補タグ（および特別ルールによるactionタグ）をカンマ区切りで並べた英語プロンプト]
[CHAT]
[感情・アクションタグ][ユーザーへのチャット本文（キャラクターの純粋なセリフのみ）]
${characterClothingRules}
【ユーザーの着衣度判定のルール】
...
現在のキャラとの会話状況・シーンを最も象徴する、画像を生成するうえで最も重要な要素（ポーズ、行為、感情、雰囲気など）を表す英語タグ（例: kissing, blushing, back-to-back, sitting on lap など）を1つだけ自由に考案して出力してください。複数のタグや説明文は出力せず、必ず1つの英語タグのみを出力してください。
${candidateTagInstruction}
...
出力例:
[excited]こんにちは！今日も会えて嬉しいわ！じゃあ、さっそくカフェに行きましょう！楽しみにしてたの～♡

上記の区切り文字列（[KEY_TAG], [IMAGE_PROMPT], [CHAT]）は、指定された位置にそれぞれ一度だけ、そのまま出力してください。`;
