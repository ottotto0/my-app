...
export async function* getGemmaResponseStream(character, userMessage, records) {
  ...
  if (hasClothingTags) {
    ...
    outputFormatInstruction = `出力は必ず次の順番・形式だけにしてください。説明や見出し、引用符などは一切出力せず、以下のフォーマットのみを出力してください。

【出力フォーマット】
[各服装タグ内容に対する着衣度（1行につき1つ「タグ内容: 数値」の形式で縦に並べる）]
...
【着衣度判定のルール】
以下の服装タグの内容について、現在の状況や会話の流れから着衣度（1 / 0.5 / 0）をそれぞれ判断し、タグ内容と着衣度をリンクさせて出力してください。
対象の服装タグ:
${targetTagsList}
...
着衣度の出力フォーマット（タグ内容: 着衣度）:
${formatExample}

上記の区切り文字列（␞␞␞CLOTHING_WEAR_LEVEL_END_8F3C␞␞␞, ␞␞␞IMAGE_PROMPT_BEGIN_8F3C␞␞␞, ␞␞␞IMAGE_PROMPT_END_8F3C␞␞␞, ␟␟␟CHAT_MESSAGE_BEGIN_8F3C␟␟␟）は、指定された位置にそれぞれ一度だけ、そのまま出力してください。`;
  } else {
    outputFormatInstruction = `出力は必ず次の順番・形式だけにしてください。画像プロンプト部分には英語のみを、チャット本文にはキャラクターとしての自然な日本語だけを入れてください。
