...
${formatExample}
`;
    delimiterList = '␞␞␞CLOTHING_WEAR_LEVEL_END_8F3C␞␞␞, ␞␞␞USER_CLOTHING_WEAR_LEVEL_BEGIN_8F3C␞␞␞, ␞␞␞USER_CLOTHING_WEAR_LEVEL_END_8F3C␞␞␞, ␞␞␞IMAGE_PROMPT_BEGIN_8F3C␞␞␞, ␞␞␞IMAGE_PROMPT_END_8F3C␞␞␞, ␟␟␟CHAT_MESSAGE_BEGIN_8F3C␟␟␟';
  } else {
    delimiterList = '␞␞␞USER_CLOTHING_WEAR_LEVEL_BEGIN_8F3C␞␞␞, ␞␞␞USER_CLOTHING_WEAR_LEVEL_END_8F3C␞␞␞, ␞␞␞IMAGE_PROMPT_BEGIN_8F3C␞␞␞, ␞␞␞IMAGE_PROMPT_END_8F3C␞␞␞, ␟␟␟CHAT_MESSAGE_BEGIN_8F3C␟␟␟';
  }

  const outputFormatInstruction = `出力は必ず次の順番・形式だけにしてください。説明や見出し、引用符などは一切出力せず、以下のフォーマットのみを出力してください。

【出力フォーマット】
${characterClothingFormat}␞␞␞USER_CLOTHING_WEAR_LEVEL_BEGIN_8F3C␞␞␞
user_cloth_level: [1 または 0]
␞␞␞USER_CLOTHING_WEAR_LEVEL_END_8F3C␞␞␞
␞␞␞IMAGE_PROMPT_BEGIN_8F3C␞␞␞
[画像生成用の英語プロンプト（外見、服装、ポーズ・表情、場所・背景、必要な視覚的要素をカンマ区切りで記述。名前、画風、品質語、説明文は含めない。英語のみ）]
␞␞␞IMAGE_PROMPT_END_8F3C␞␞␞
␟␟␟CHAT_MESSAGE_BEGIN_8F3C␟␟␟
[ユーザーへのチャット本文（キャラクターとしての自然な日本語のみ）]
${characterClothingRules}
【ユーザーの着衣度判定のルール】
現在の状況や会話の流れから、ユーザーの着衣度を必ず判断し、次の2択（1 または 0）で「user_cloth_level: 数値」を出力してください。
1（完全着衣を示す。）
0（未着衣を示す。）

上記の区切り文字列（${delimiterList}）は、指定された位置にそれぞれ一度だけ、そのまま出力してください。`;
