...
async function getGemmaCandidates() {
  ...
  return { candidates, spaceName };
}

async function markTokenUsed(selected) {
...
async function createGemmaClient() {
  const { candidates, spaceName } = await getGemmaCandidates();
  const selected = candidates.find(c => c !== null);

  if (selected) {
    const hfToken = selected.token;
    console.log(`Using token ${selected.name || selected.id} for Gemma authentication.`);
    await markTokenUsed(selected);
...
function getReplyError(data) {
  if (!data) return '';
  const findError = (value, visited = new Set()) => {
    ...
    if (Array.isArray(value)) {
      for (const item of value) {
        const err = findError(item, visited);
        if (err) return err;
      }
      return '';
    }
    ...
    for (const child of Object.values(value)) {
      const err = findError(child, visited);
      if (err) return err;
    }
    return '';
  };

  return findError(data);
}
...
function getReplyContent(data) {
  ...
  const findText = (value, visited = new Set()) => {
    ...
    // キー以外にもテキストを探索する（role / ファイルのメタデータ / エラーは除外）。
    for (const [key, child] of Object.entries(value)) {
      if (['role', 'url', 'path', 'name', 'mime_type', 'size', 'error'].includes(key)) continue;
...
export async function completeImagePrompt({ supabase, character, wearSection, rawImagePrompt, keyTag }) {
  ...
  const tagsList = [];
  if (keyTag && typeof keyTag === 'string' && keyTag.trim()) {
    const cleanKeyTag = keyTag.replace(/^["'`]+|["'`]+$/g, '').trim();
    if (cleanKeyTag) {
      tagsList.push(cleanKeyTag);
    }
  }
  tagsList.push(...initialTags);
  ...
  if (currentTagSet.has('front view')) {
    ...
    addDeletionCategory('ass');
  }
  if (currentTagSet.has('back view')) {
    addDeletionCategory('bangs style');
  }

  // ユーザーの着衣度が0の場合、charactersテーブルのmale_pubic_hairとpenisを追加
  const userWearMatch = (wearSection || '').match(/user[\s_]*wear\s*:\s*\[?([0-9.]+)\]?/i);
  const userWearScore = userWearMatch ? parseFloat(userWearMatch[1]) : null;
  if (isScore(userWearScore, 0)) {
    const malePubicHair = charData ? charData.male_pubic_hair : null;
    if (typeof malePubicHair === 'string' && malePubicHair.trim()) {
      tagsList.push(malePubicHair.trim());
    }
    const penis = charData ? charData.penis : null;
    if (typeof penis === 'string' && penis.trim()) {
      tagsList.push(penis.trim());
    }
  }
...
export async function* getGemmaResponseStream(character, userMessage, records) {
  ...
  const previousImagePromptInstruction = previousImagePrompt
    ? `\n直前に生成した画像プロンプトです。場面・服装・ポーズなどがどう変化したのかを考えるために参考にしてください。\n${previousImagePrompt}\n`
    : '';
...
画像生成プロンプトでは、最重要タグと「action」カテゴリーのタグ以外は、タグを1から自由に生成することは禁止です。
...
【出力フォーマット（厳守）】
[CHAT]
[ここに感情タグが入る。]ここにキャラクター自身のセリフ本文が入る。
${characterClothingFormat}user wear: ここに1か0が入る。
[KEY_TAG]
ここに最重要タグが入る。
[IMAGE_PROMPT]
ここに各categoryごとに、そのcategoryのtagの中から一つずつ入る。

※重要注意事項:
1. 「[CHAT]」「[KEY_TAG]」「[IMAGE_PROMPT]」の3つのタグ文字列は、セクション区切りのためのキーワードです。絶対に別の言葉に置き換えたり省略したり角括弧を増やしたりせず、そのまま正確に1行ずつ出力してください。
2. [CHAT] の次の行には、必ず[感情タグ]で開始し、それに続けてキャラクター自身のセリフ本文のみを出力してください。
3. セリフを出力し終えたら、改行してキャラの各服装の着衣度および「user wear: 数値」を出力してください。
4. [KEY_TAG] の次の行には、英語タグを1つだけ出力してください（角括弧で囲まないでください）。
5. [IMAGE_PROMPT] の次の行には、候補タグをカンマ区切りで出力してください（角括弧で囲まないでください）。

【キャラのチャット発言（[CHAT]）の生成ルール】
1. キャラの発言の直前（[CHAT]の直後、発言の先頭）に、必ず以下の【感情・アクションタグ】の選択肢から、現在の感情やニュアンスに最もふさわしいタグを1つ選択して出力してください。
感情・アクションタグ選択肢:
- [whispers] (ささやき)
- [sighs] (ため息)
- [laughs] (笑い)
...
2. 音声合成で直接読み上げるため、状況の説明文やト書き（例: 「（微笑みながら）」「〜と言った。」などの地の文や行動描写）は一切含めず、完全にキャラクター自身の「セリフ（発言）」のみを出力してください。
${characterClothingRules}
【ユーザーの着衣度判定のルール】
...
【最重要タグ（[KEY_TAG]）の生成ルール】
現在のキャラとの会話状況・シーンを最も象徴する、画像を生成するうえで最も重要な要素（ポーズ、行為、感情、雰囲気など）を表す英語タグ（例: kissing, blushing, back-to-back, sitting on lap など）を1つだけ考案して出力してください。角括弧は付けず、タグの単語のみを出力してください。
${candidateTagInstruction}`;
...
${outputFormatInstruction}`;
  ...
  for (let cIdx = 0; cIdx < candidates.length; cIdx++) {
    ...
    const candidateLabel = candidate ? (candidate.name || candidate.id) : 'anonymous';

    if (candidate) {
      console.log(`Using token ${candidateLabel} for Gemma authentication.`);
    } else {
      console.log(`Using anonymous access for Gemma authentication.`);
    }
    ...
    try {
      ...
      console.warn(`Connection to Gemma failed with candidate "${candidateLabel}":`, connErr?.message || connErr);
      lastError = connErr;
      continue;
    }
    ...
    try {
      ...
      for await (const event of submission) {
        ...
        const errText = getReplyError(event.data);
        if (errText) {
        ...
        if (delta) {
          if (!hasYielded && candidate) {
            await markTokenUsed(candidate);
          }
          hasYielded = true;
          yield delta;
        }
      ...
      console.warn(`Gemma call failed with candidate "${candidateLabel}":`, err?.message || err);
      lastError = err;
  ...
  throw new Error(`全てのGemma認証候補で生成に失敗しました: ${lastError?.message || '不明なエラー'}`);
}
