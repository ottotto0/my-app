...
※重要注意事項:
1. 「[KEY_TAG]」「[IMAGE_PROMPT]」「[CHAT]」の3つのタグ文字列は、セクション区切りのためのキーワードです。絶対に別の言葉に置き換えたり省略したり角括弧を増やしたりせず、そのまま正確に1行ずつ出力してください。
2. [KEY_TAG] の次の行には、英語タグを1つだけ出力してください（角括弧で囲まないでください）。
3. [IMAGE_PROMPT] の次の行には、候補タグをカンマ区切りで出力してください（角括弧で囲まないでください）。
4. [CHAT] の次の行には、必ず[感情タグ]で開始し、それに続けてキャラクター自身のセリフ本文のみを出力してください。
${characterClothingRules}
【ユーザーの着衣度判定のルール】
